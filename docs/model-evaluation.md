...
Each result includes immutable Hugging Face revision, runtime package versions,
...
|---|---|---|---|---|
| Baseline evaluation | tiny v3 | 6.68% | 0.059 | [Baseline](model-loop/20260916-tiny-to-base/baseline.json) |
| Candidate evaluation | base v4 → staging | 6.68% | 0.104 | [Candidate](model-loop/20260916-tiny-to-base/candidate.json) |
| Fresh evaluation and promotion | base v4 → production | 6.68% | 0.127 | [Release receipt](model-loop/20260916-tiny-to-base/release.json) |
| Deploy promoted base image | stable container uses candidate digest | — | — | [Deployment receipt](model-loop/20260916-tiny-to-base/deployment.json) |
| Fresh deployed verification | base, production v4 | 6.68% | 0.128 | [Verification](model-loop/20260916-tiny-to-base/verify.json) |
| Roll back registry and deployment | tiny, production v3 | — | — | [Registry rollback](model-loop/20260916-tiny-to-base/rollback.json), [image rollback](model-loop/20260916-tiny-to-base/rollback-deployment.json) |
| Fresh rollback verification | tiny, production v3 | 6.68% | 0.076 | [Verification](model-loop/20260916-tiny-to-base/verify-rollback.json) |
...
The [updated CI command](model-loop/20260916-tiny-to-base/ci-check.log) was also
...
Reproduce in a fresh directory with `docs/model-loop/run-demo.ps1`.

The final models-from-code registration implementation also completed all five
phases in a [fresh registry](model-loop/20260916-portable-model-check/verify-rollback.log).
A separate [artifact loading check](model-loop/20260916-portable-model-check/portable-pyfunc.json)
