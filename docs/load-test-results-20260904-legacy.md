...
> **Historical narrative, superseded by [the repaired measurement](load-test-results.md).**
> These runs lacked a final task census, deployed revision/digest provenance, and
> queue/processing timestamps. Their client also read only one WebSocket message.
> Keepalive errors and abandoned jobs therefore prevent these results from
> establishing successful end-to-end capacity. The HTML, request CSV, and failure
> CSV are different snapshots; their totals must not be combined. Paths and bug
> status below describe the old deployment and are retained as historical notes.
